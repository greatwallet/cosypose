...
def h_pose_custom(model, mesh_db, data, meters,
           cfg, n_iterations=1, input_generator='fixed'):

    batch_size, _, h, w = data.src_pose.images.shape

    # src
    src_img_bs = cast(data.src_pose.images).float() / 255.
    K = cast(data.src_pose.K).float()
    src_TCO_gt = cast(data.src_pose.TCO).float()
    labels = np.array([obj['name'] for obj in data.src_pose.objects])
    src_bboxes = cast(data.src_pose.bboxes).float()

    # dest
    dest_img_bs = cast(data.dest_pose.images).float() / 255.
    dest_TCO_gt = cast(data.dest_pose.TCO).float()
    dest_bboxes = cast(data.dest_pose.bboxes).float()

    meshes = mesh_db.select(labels)
    points = meshes.sample_points(cfg.n_points_loss, deterministic=False)
    # TCO_possible_gt = src_TCO_gt.unsqueeze(1) @ meshes.symmetries
    TCO_possible_gt = dest_TCO_gt

    if input_generator == 'fixed':
        TCO_init = TCO_init_from_boxes(z_range=(1.0, 1.0), boxes=src_bboxes, K=K)
    elif input_generator == 'gt+noise':
        TCO_init = add_noise(TCO_possible_gt[:, 0], euler_deg_std=[15, 15, 15], trans_std=[0.01, 0.01, 0.05])
    elif input_generator == 'fixed+trans_noise':
        assert cfg.init_method == 'z-up+auto-depth'
        TCO_init = TCO_init_from_boxes_zup_autodepth(src_bboxes, points, K)
        TCO_init = add_noise(TCO_init,
                             euler_deg_std=[0, 0, 0],
                             trans_std=[0.01, 0.01, 0.05])
    else:
    ...
    # model.module.enable_debug()
    outputs = model(
        src_img_bs=src_img_bs, 
        dest_img_bs=dest_img_bs, 
        K=K, 
        labels=labels, 
        TCO=TCO_init, 
        n_iterations=1
