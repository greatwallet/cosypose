...
from cosypose.config import LOCAL_DATA_DIR, ASSET_DIR, BOP_DS_DIR, KWAI_PATH
...
from .kwai import KwaiDataset, KwaiTrackingDataset, KwaiObjectDataset
...
def make_scene_dataset(ds_name, n_frames=None):
    ...
    # Kwai datasets
    elif "kwai" in ds_name:
        ds_dir = KWAI_PATH
        split = ds_name.split('.')[-1]
        if "tracking" in ds_name:
            ds = KwaiTrackingDataset(ds_dir, split)
        else:
            ds = KwaiDataset(ds_dir, split) 
