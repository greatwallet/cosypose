...
class KwaiPosePredictor(nn.Module):
    ...
    def crop_inputs(self, images, K, TCO, labels):
        ...
        points = meshes.sample_points(2000, deterministic=True)
        uv = project_points(points, K, TCO)
        boxes_rend = boxes_from_uv(uv) # point set to find bbox hull
        boxes_crop, images_cropped = deepim_crops(
            images=images, obs_boxes=boxes_rend, K=K,
            TCO_pred=TCO, O_vertices=points, output_size=self.render_size, lamb=1.4
        )
    ...
    def forward(self, src_img_bs, dest_img_bs, K, labels, TCO, n_iterations=1):
        bsz, nchannels, h, w = src_img_bs.shape
        assert K.shape == (bsz, 3, 3)
        assert TCO.shape == (bsz, 4, 4)
        assert len(labels) == bsz
        assert dest_img_bs.shape == src_img_bs.shape

        outputs = dict()
        TCO_input = TCO
        # the method could only be 1, since there is no rendering involved
        assert n_iterations == 1
        for n in range(n_iterations):
            TCO_input = TCO_input.detach()
            # TODO: check K_crop is the same?
            src_bs_crop, K_crop, boxes_rend, boxes_crop = self.crop_inputs(src_img_bs, K, TCO_input, labels)
            dest_bs_crop, K_crop, boxes_rend, boxes_crop = self.crop_inputs(dest_img_bs, K, TCO_input, labels)
            # renders = self.renderer.render(obj_infos=[dict(name=l) for l in labels],
            #                                TCO=TCO_input,
            #                                K=K_crop, resolution=self.render_size)

            x = torch.cat((src_bs_crop, dest_bs_crop), dim=1)
            ...
            if self.debug:
                raise NotImplementedError("Debug mode not implemented for kwai")
                # self.tmp_debug.update(outputs[f'iteration={n+1}'])
                # self.tmp_debug.update(
                #     images=images,
                #     images_crop=images_crop,
